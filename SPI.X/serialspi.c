...
int main()
{
	register int i;
	int fd_serie;
	unsigned char ejeX0, ejeX1, ejeY0, ejeY1, ejeZ0, ejeZ1;
	short int ejeX, ejeY, ejeZ;
	fd_serie = config_serial( "/dev/ttyUSB0", B57600 );
	printf("Serial abierto con descriptor: %d\n", fd_serie);

	//Leemos N datos del UART
	while(1)
	{
		read( fd_serie, &ejeX0, 1 );
		read( fd_serie, &ejeX1, 1 );
		ejeX = (ejeX1 << 8) | ejeX0;

		read( fd_serie, &ejeY0, 1 );
		read( fd_serie, &ejeY1, 1 );
		ejeY = (ejeY1 << 8) | ejeY0;

		read( fd_serie, &ejeZ0, 1 );
		read( fd_serie, &ejeZ1, 1 );
		ejeZ = (ejeZ1 << 8) | ejeZ0;
		
		printf("Eje X: %d , Eje Y: %d, Eje Z: %d \n", ejeX, ejeY, ejeZ );
		clock_t start = clock();
